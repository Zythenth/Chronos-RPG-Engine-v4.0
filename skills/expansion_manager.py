...
_DYNAMIC_ITEMS_PATH = os.path.join(_CTX_DIR, "dynamic_items.json")
...
def _load_tiers() -> tuple[dict, dict]:
    ...
    if not _spec or not _spec.loader:
        raise RuntimeError("mechanics_engine.py não encontrado")
    _me = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(_me)
    ct = getattr(_me, "CREATURE_TIERS", {})
    it = getattr(_me, "ITEM_TIERS", {})
    if not ct or not it:
        raise ValueError("Os limitadores de TIER não existem ou falharam (fonte: mechanics_engine.py).")
    return ct, it
...
def check_entity_exists(name: str, entity_type: str) -> bool:
    ...
    if entity_type in ("criatura", "creature"):
        ...
        # Verifica loot_manager.py, dynamic_items.json e inventory.csv
        paths_to_check = [
...
def save_new_item(data: dict) -> bool:
    """
    Salva novo item em world_context/dynamic_items.json.
    loot_manager.py carrega esse registro no import e expõe os itens em ITEM_SCHEMA.
    """
    ...
    try:
        ...
        if os.path.exists(_DYNAMIC_ITEMS_PATH):
            with open(_DYNAMIC_ITEMS_PATH, encoding="utf-8") as f:
        ...
        items[nome] = {
            ...
            "weight_kg":       item_tier.get("peso_kg", 0.5),
            "effect":          data.get("efeito", "Efeito desconhecido."),
            "usable":          bool(data.get("usable", False)),
            "notes":           data.get("notas", "Gerado pelo PROTOCOLO DE EXPANSÃO."),
            "durabilidade":    item_tier.get("durabilidade"),
            "quantidade_drop": item_tier.get("quantidade_drop"),
            "source":          "expansion_manager",
        }
        registry["_schema"] = "items[name] -> loot_manager.ITEM_SCHEMA-compatible item definition"

        tmp = f"{_DYNAMIC_ITEMS_PATH}.tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(registry, f, ensure_ascii=False, indent=2)
        os.replace(tmp, _DYNAMIC_ITEMS_PATH)
    except Exception as e:
        print(f"ERRO: falha ao salvar dynamic_items.json: {e}")
        return False

    print(f"  ✓ Item '{nome}' salvo em world_context/dynamic_items.json")
    return True

...
def save_character_sheet(cs: dict) -> None:
    _atomic_json_write(_CS_PATH, cs, indent=2)

def save_active_combat(ac: dict) -> None:
    _atomic_json_write(_AC_PATH, ac, indent=2)
...
def save_chapter_tracker(ct: dict) -> None:
    _atomic_json_write(_CT_PATH, ct, indent=4)
...
def action_combat(cs: dict, ac: dict, args, passive_fx: dict, report: list) -> None:
    ...
    Combate melhorado com:
      ...
      - Flanqueio real: +_me.FLANQUEAR_DAMAGE_BONUS de dano direto
      - Multi-ataque: nivel >= _me.MULTI_ATTACK_MIN_LEVEL permite segundo ataque com penalidade
    """
    ...
    if level >= _me.MULTI_ATTACK_MIN_LEVEL and out1 != "FALHA CRÍTICA":
        report.append(f"   ★ MULTI-ATAQUE (Nível {level}): segundo ataque com {_me.MULTI_ATTACK_PENALTY:+}")
        out2, dmg2, _, _, _, _ = _single_attack(_me.MULTI_ATTACK_PENALTY)
    ...
    if not inimigo_morto:
        ...
        if hp_pct <= _me.MORAL_FLEE_THRESHOLD and tem_moral:
            roll_moral = _d20.rolar_d20()
            dc_moral   = inimigo.get("ficha_racial", {}).get("dc_moral", 10)
            if roll_moral < dc_moral:
                inimigo_fugiu = True
                report.append(f"   ⚠ MORAL QUEBRADA: {inimigo.get('nome','?')} HP={hp_pct*100:.0f}% ≤ {_me.MORAL_FLEE_THRESHOLD*100:.0f}%")

...
_DYNAMIC_ITEM_PATH = os.path.join(_PROJ, "world_context", "dynamic_items.json")
...
def _load_dynamic_item_schema() -> dict:
    """Load generated item definitions without mutating Python source code."""
    if not os.path.exists(_DYNAMIC_ITEM_PATH):
        return {}
    try:
        with open(_DYNAMIC_ITEM_PATH, encoding="utf-8") as f:
            raw = json.load(f)
    except Exception:
        return {}
    ...
    if isinstance(raw, dict) and isinstance(raw.get("items"), dict):
        ...
        # Legacy/fallback shape: {"Item": {schema...}}
        candidates = {k: v for k, v in raw.items() if not str(k).startswith("_")}
    else:
        return {}
    ...
    for name, schema in candidates.items():
        if not isinstance(name, str) or not name.strip() or not isinstance(schema, dict):
            continue
        dynamic[name.strip()] = {
            "type":      str(schema.get("type", "Material")),
            "rarity":    str(schema.get("rarity", "Raro")),
            "weight_kg": float(schema.get("weight_kg", 0.5) or 0.5),
            "effect":    str(schema.get("effect", "")),
            "usable":    bool(schema.get("usable", False)),
            "notes":     str(schema.get("notes", "Gerado pelo expansion_manager.")),
        }
        if "durabilidade" in schema:
            dynamic[name.strip()]["durabilidade"] = schema.get("durabilidade")
        if "quantidade_drop" in schema:
            dynamic[name.strip()]["quantidade_drop"] = schema.get("quantidade_drop")
    return dynamic


ITEM_SCHEMA.update(_load_dynamic_item_schema())

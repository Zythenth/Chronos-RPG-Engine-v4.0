...
def _query_terms(query: str) -> set[str]:
    """Extrai termos simples para recuperar blocos de memória por relevância."""
    import re as _re
    stopwords = {
        "com", "sem", "para", "por", "que", "uma", "umas", "uns", "dos", "das",
        "como", "acao", "ação", "turno", "parte", "resultado", "jogador",
        "ferro", "chronos", "sistema", "relatorio", "técnico", "tecnico",
    }
    terms = {
        t.lower()
        for t in _re.findall(r"[A-Za-zÀ-ÿ0-9_-]{4,}", query or "")
        if t.lower() not in stopwords
    }
    return terms


def load_story_bible_relevant(query: str, max_chars: int = 2000) -> str:
    """
    Carrega blocos da story_bible relacionados à ação atual.
    Fallback: últimas entradas, preservando o comportamento anterior.
    """
    raw: str = _read(PATHS["story_bible"], "(story_bible.md vazia)")
    ...
    if not terms:
        return load_story_bible_recent(max_chars=max_chars)
    ...
    selected: list[str] = []
    total = 0
    for _, _, block in sorted(scored, key=lambda x: (x[0], x[1]), reverse=True):
        extra = len(block) + 8
    ...
    selected.reverse()
    return "[memória relevante por termos]\n\n" + "\n\n---\n".join(selected)
...
def build_world_context_for_gm(query: str = "") -> str:
    """
...
── HISTÓRIA RELEVANTE/RECENTE (story_bible) ────────────
{load_story_bible_relevant(query, max_chars=1500)}

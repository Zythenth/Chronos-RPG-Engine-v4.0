...
def save_json(path: str, data: dict) -> None:
    if os.path.basename(path) == "character_sheet.json":
        ...
        if erros:
            ...
            raise ValueError(f"FALHA DE VALIDAÇÃO no character_sheet: {erros}")
...
def _parse_bestiary(path: str) -> dict:
    ...
    for block in blocks:
        ...
        veloc_raw = _get(r'\*\*Velocidade:\*\*\s*(\d+)')
        ...
        dc_moral_raw = _get(r'\*\*DC de Moral:\*\*\s*(\d+)')
        ...
        result[nome_canon] = {
            ...
            "velocidade":   int(veloc_raw) if veloc_raw else 2,
            ...
            "ficha_racial": {
                ...
                "pode_fugir":      threshold.lower() != 'nunca',
                "dc_moral":        int(dc_moral_raw) if dc_moral_raw else 10,
            },
...
# ── 5.8 LEVEL UP EXPLICITO (novo) ─────────────────────────────────────────

def cmd_level_up(args) -> None:
    cs = load_json(_CS_PATH)
    prog = cs.get("progression", {})
    ...
    if lv_result["level_up"]:
        ...
        save_json(_CS_PATH, cs)
        print(f"\n★ LEVEL UP APLICADO → Nível {new_lv}!")
...
def main():
    ...
    # level_up
    sub.add_parser("level_up", help="Aplica nivelamento de XP, se disponível")
